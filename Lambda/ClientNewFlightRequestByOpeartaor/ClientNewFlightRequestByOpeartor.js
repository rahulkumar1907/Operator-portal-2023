...
wf.once('get_organizationID_from_ClientProfile', function () {
    ...
    wf.once('request_new_flight_with_different_time_slot_role_not_manager', function () {
        //@RK - 13/04/23 - Event will change the status to approval pending if role is not manager
        console.log("coming")
                ...
                docClient.update({
                    ...
                    if (err) {
                        ...
                        docClient.put(params, function (err, data) {
                            //@RK - 13/04/23 - If request is success then send an email to manager
                            if (err) {
                                console.log("error7")
                                console.log(err);
                                context.fail(JSON.stringify({
                                ...
                                //@RK - 14/04/23 - Added package number field to template
                                //@RK - 20/04/23 - Removed flight id from email template
                                //@RK - 20/04/23 - Removed flight id from email template
                                let params = {
                                    "Source": FromAddress,
    ...
    wf.once('request_new_flight_with_different_time_slot_role_manager', function () {
        //@RK - 13/04/23 - Event will auto approve request if role is manager
        let PickupDetails = event.body.PickupDetails
        let i = 0;
