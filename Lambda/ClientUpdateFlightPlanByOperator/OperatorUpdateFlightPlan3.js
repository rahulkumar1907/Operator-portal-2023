...
var AdminApproveFlightRequestLambda1 = process.env.AdminApproveFlightRequestLambda;
...
exports.handler = (event, context, callback) => {
    ...
    // STORING EMAIL OF OPERATION MANAGER
    let OperatorEmails = [];

    wf.once('check_request_body', function () {
        // VALIDATING ALL REQUIRED REQUEST BODY 
        var parameters = '/';
    ...
    wf.once('get_waypoint_details_from_skyetunnel', function () {
        ...
        // GETTING DATA TO UPDATE FOR FLIGHT FROM SKYTUNNEL TABLE
        var params = {
        ...
        docClient.get(params, function (err, data) {
            if (err) {
                ...
                // DATA NEEDED TO STORE FROM SKYTUNNEL TO FLIGHT
                ...
                for (let i = 0; i < data.Item.Drone.length; i++) {
                    if (data.Item.Drone[i].DroneID == event.body.DroneId) {
                        wf.updatedFlightDetails.DroneName = data.Item.Drone[i].DroneName;
                        wf.updatedFlightDetails.DronePlatform = data.Item.Drone[i].DronePlatform;
                        wf.updatedFlightDetails.PayLoadCapacity = data.Item.Drone[i].PayLoadCapacity;
                    }
                }
                // GETTING EMAIL OF OPERATION MANAGER
    ...
    wf.once('get_oldFlightDetails', function () {
        // GETTING OLD FLIGHT DATA THAT NEED TO CANCELLED
        const params = {
    ...
    wf.once('register_flightDetails_to_table', function () {
        // CREATING NEW REQUEST FOR FLIGHT TO APPROVE
        var params = {
            "TableName": FlightTable,
        ...
        if (event.body.Role == "Operation Manager") {
            wf.updatedFlightDetails.Status = "Scheduled";
            let params = {
                "TableName": FlightTable,
                Item: wf.updatedFlightDetails
            };
                ...
                if (err) {
                    ...
                    ses.sendTemplatedEmail(params, function (err, data) {
                        if (err) {
                            ...
                            let payload = {
                                DroneId: event.body.DroneId, TakeOffPilot: event.body.TakeOffPilot, LandingPilot: event.body.LandingPilot, FlightId: wf.updatedFlightDetails.FlightId, ClientName: wf.updatedFlightDetails.ClientName
                            };

                            const params = {
                                FunctionName: AdminApproveFlightRequestLambda1, // Replace 'my-function' with the name of your Lambda function
